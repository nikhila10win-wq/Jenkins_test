...
@Component(
	property = {
		...
		"javax.portlet.display-name=MhdsysAwardYouthWeb",
		"javax.portlet.init-param.template-path=/",
		"javax.portlet.init-param.view-template=" + YouthAwardConstants.AWARD_YOUTH,
...
public class MhdsysAwardYouthWebPortlet extends MVCPortlet {
	private Log LOGGER=LogFactoryUtil.getLog(MhdsysAwardYouthWebPortlet.class);
	 ...
	 public void render(RenderRequest renderRequest, RenderResponse renderResponse)
	     ...
	     try {
	         ...
	         AwardYouthCommonDTO awardYouthCommonDTO = null;
	         String mode = "add"; 

	         AwardYouth draftEntry = fetchAwardYouthBaseOnFormStatusAndUserId(userId, "Draft");
	         if (Validator.isNotNull(draftEntry)) {
	             awardYouthCommonDTO = youthAwardCommonUtil.setAwardYouthCommonDTO(draftEntry, themeDisplay);
	             LOGGER.info("awardYouthCommonDTO:::::::::"+ awardYouthCommonDTO);
	             mode = "edit";
	         } else {
	             AwardYouth submittedEntry = fetchAwardYouthBaseOnFormStatusAndUserId(userId, "Submitted");
	             if (Validator.isNotNull(submittedEntry)) {
	                 awardYouthCommonDTO = youthAwardCommonUtil.setAwardYouthCommonDTO(submittedEntry, themeDisplay);
	                
	                 mode = "view";
	             }
	         }

	         if (Validator.isNotNull(awardYouthCommonDTO)) {
	...
	private AwardYouth fetchAwardYouthBaseOnFormStatusAndUserId(long userId,String formStatus) {
		...
		List<AwardYouth> results = AwardYouthLocalServiceUtil.dynamicQuery(dynamicQuery);
