...
@Component(property = { "com.liferay.portlet.display-category=MHDSYS.Competition",
		...
		"javax.portlet.display-name=MhdsysPrincipalApproveListManagementWeb",
		...
		"javax.portlet.init-param.view-template=" + CompetitionCommonConstant.PRINCIPAL_APPROVE_LIST_JSP,
		"javax.portlet.name="
				+ CompetitionManagementWebPortletKeys.MHDSYS_COMPETITION_PRINCIPAL_APPROVED_LIST_MANAGEMENTWEB,
		"javax.portlet.resource-bundle=content.Language",
		"javax.portlet.security-role-ref=power-user,user" }, service = Portlet.class)
public class PrincipalApprovedListCompetitionWebPortlet extends MVCPortlet {
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		try {
			...
			LOGGER.info("isDSORole: " + isDSORole);
			List<Long> statuses;
			List<PTTeacherApplicationCommonDTO> ptTeacherApplicationCommonDTOs = new ArrayList<>();
			List<PTTeacherApplication> ptTeacherApplications = null;
			if (isPrincipal) {
				statuses = Arrays.asList(CompetitionCommonConstant.PROCESS1_APPROVED_BY_PRINCIPAL,
						CompetitionCommonConstant.PROCESS1_FORWARDED_TO_DSO,
						CompetitionCommonConstant.PROCESS1_APPROVED_BY_DSO,
						CompetitionCommonConstant.PROCESS2_APPROVED_BY_PRINCIPAL);
				ptTeacherApplications = PTTeacherApplicationLocalServiceUtil.getPtTeacherFormByStatus(statuses);
			}
			if (isDSORole) {
				statuses = Arrays.asList(CompetitionCommonConstant.PROCESS1_APPROVED_BY_DSO,
						CompetitionCommonConstant.PROCESS1_FORWARDED_TO_DSO,
						CompetitionCommonConstant.PROCESS2_APPROVED_BY_PRINCIPAL);
				ptTeacherApplications = PTTeacherApplicationLocalServiceUtil.getPtTeacherFormByStatus(statuses);
				LOGGER.info("ptTeacherApplications size: " + ptTeacherApplications.size());
			}
