			...
			<div class="col-md-12">
				<div class="border-0 card shadow">
					<div class="align-items-center back-btn-cn card-header d-flex justify-content-between">
						<h5><liferay-ui:message key="pt-teacher-application"/></h5>	
                ...
                <tbody>
                    <c:forEach var="ptTeacherApplication" items="${ptTeacherApplications}">
                        ...
                        <portlet:renderURL var="competitionScheduleURL">
                        ...
                        <tr>
                         <c:if test="${isPrincipal || isDSORole}">
                            <td>
                                ...
                                <c:if test="${(isPrincipal && (ptTeacherApplication.status == 3 || ptTeacherApplication.status == 4 || ptTeacherApplication.status == 6)) || 
                                (isDSORole && (ptTeacherApplication.status == 4 || ptTeacherApplication.status == 6))}">disabled</c:if> />
                            </td>
                         </c:if>
                            <td>${ptTeacherApplication.sportName} ${ptTeacherApplication.competitionInitiationId}</td>
                            ...
                            <td><a href="${viewPtTeacherApplicationURL}" class="btn btn-primary"><i class="bi bi-eye icons-color" title="<liferay-ui:message key="view" />"></i></a>
                            <c:if test="${isDSORole && ptTeacherApplication.status == 6 && !ptTeacherApplication.scheduled}">
                            	<a href="${competitionScheduleURL}" class="btn btn-primary"><i class="bi bi-calendar2-plus icons-color" title="<liferay-ui:message key="schedule" />"></i>
                            </c:if>
                            </td>
                        </tr>
                    </c:forEach>
	        ...
	        <div class="row mt-3">
	            <div class="col-md-12 text-right">
	                <c:if test="${isPrincipal }">
	                	<button class="submitBtn btn btn-primary" disabled><liferay-ui:message key="send-to-dso" /></button>
	                </c:if>
	                <c:if test="${isDSORole }">
	                	<button class="submitBtn btn btn-primary" disabled><liferay-ui:message key="approve" /></button>
	                </c:if>
	            </div>
	        </div>
	    </form>
						...
						<div class="row">
							<div class="col-md-12 text-center">
									<div>
                                   	 ...
                                   	 <c:if test="${isPrincipal }">
                                   		 <p><liferay-ui:message key="forwarded-to-dso-is-successful"/></p>
                                     </c:if>
                                     <c:if test="${isDSORole }">
                                         <p><liferay-ui:message key="approved-by-dso-is-successful"/></p>
                                     </c:if>
									</div>
							</div>
						</div>
	...
	var table = $('#principal-approve-list').DataTable({
	 ...
	 // Search
	    $('#searchBtn').click(function (e) {
	        ...
	        if (sport_name) table.column(0).search(sport_name);
	        if (count_of_team_or_individual) table.column(1).search(count_of_team_or_individual);
	        if (total_fees) table.column(2).search(total_fees);
	        if (status) table.column(3).search(status);
	        table.draw();
	    });

});
 
 var dsoApproval = '${isDSORole}';
        ...
        $('.submitBtn').on('click', function (event) {debugger
            ...
            formData.append('dsoApproval', dsoApproval);
            // AJAX call
            $.ajax({
                type: "POST",
                url: '${sendToDSOFormURL}',
                data: formData,
                contentType: false,
                cache: false,
                processData: false,
