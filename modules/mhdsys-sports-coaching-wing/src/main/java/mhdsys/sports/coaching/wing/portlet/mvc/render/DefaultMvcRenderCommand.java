...
public class DefaultMvcRenderCommand implements MVCRenderCommand {
	...
	public static final Log _log=LogFactoryUtil.getLog(DefaultMvcRenderCommand.class.getName());
	
	@Override
	public String render(RenderRequest renderRequest, RenderResponse renderResponse) throws PortletException {
		_log.info("DefaultMvcRenderCommand  render ::");
		
		String redirectPage="/view.jsp";
		...
		ThemeDisplay themeDisplay = (ThemeDisplay)renderRequest.getAttribute(WebKeys.THEME_DISPLAY);
		 ...
		 _log.info("isCoach :::" +isCoach);
		 _log.info("ishoadmin :::" +ishoadmin);
		 String friendlyURL = themeDisplay.getLayout().getFriendlyURL();
		 _log.info("friendlyURL :::" +friendlyURL);
		 
		if(friendlyURL.equalsIgnoreCase("/sports-coaching-wing")){
			 redirectPage="/page-2.jsp";
			if(isCoach) {
				redirectPage="/reports.jsp";
			...
			return redirectPage;
		}else if(friendlyURL.equalsIgnoreCase("/sports-coaching-wing-reports")) {
			_log.info("Fina Redirect for ");
		if(isCoach) {
					renderRequest.setAttribute("isCoach",true);
				}else if(isDSO) {
					renderRequest.setAttribute("isDSO",true);
			...
			return "/reports.jsp";
		}else if(friendlyURL.equalsIgnoreCase("/sports-coaching-wing-monthly-reports")) {
				...
				if(Validator.isNotNull(coach)) {
					...
					if(Validator.isNotNull(coachingWingMonthlyReportsList) && coachingWingMonthlyReportsList.size()>0) {
						for(CoachingWingMonthlyReport report :coachingWingMonthlyReportsList) {
							...
							}else if(report.getStatus().equalsIgnoreCase(MhdsysSportsCoachingWingPortletKeys.REJECED_BY_DSO)) {
								dto.setStatus("Rejected By DSO");
							}
							reportDTOList.add(dto);
						}
					}
		...
		}else if(friendlyURL.equalsIgnoreCase("/district-sport-talent-development-centre-reports")) {
			...
			List<CoachingWingFinancialDetailsReportsDTO> type1Report = getTrainingCenterFinancialReportByTrainingCenterType("talent_dev");
			_log.info("districts :::" +districtList);
			renderRequest.setAttribute("centerType","talent_dev");
			renderRequest.setAttribute("talentDev",type1Report);
			 renderRequest.setAttribute("districts",districtList);
			return "/coaching-wing-fiancial-detail-reports.jsp";
			
		}else if(friendlyURL.equalsIgnoreCase("/district-coaching-centre-report")) {
			List<DistrictMaster> districtList = DistrictMasterLocalServiceUtil.getByActiveState(Boolean.TRUE);
			renderRequest.setAttribute("districts", districtList);
			//District Sport Talent Development Centre
			List<CoachingWingFinancialDetailsReportsDTO> type1Report = getTrainingCenterFinancialReportByTrainingCenterType("coaching");
			_log.info("districts :::" +districtList);
			renderRequest.setAttribute("centerType","coaching");
			 renderRequest.setAttribute("talentDev",type1Report);
			 renderRequest.setAttribute("districts",districtList);
			return "/coaching-wing-fiancial-detail-reports.jsp";
			
		}else if(friendlyURL.equalsIgnoreCase("/khelo-india-district-centre-report")) {
			List<DistrictMaster> districtList = DistrictMasterLocalServiceUtil.getByActiveState(Boolean.TRUE);
			renderRequest.setAttribute("districts", districtList);
			//District Sport Talent Development Centre
			List<CoachingWingFinancialDetailsReportsDTO> type1Report = getTrainingCenterFinancialReportByTrainingCenterType("khelo_india");
			_log.info("districts :::" +districtList);
			renderRequest.setAttribute("centerType","khelo_india");
			 renderRequest.setAttribute("talentDev",type1Report);
			 renderRequest.setAttribute("districts",districtList);
			return "/coaching-wing-fiancial-detail-reports.jsp";
		}else if(friendlyURL.equalsIgnoreCase("/taluka-coaching-centre-report")) {
			List<DistrictMaster> districtList = DistrictMasterLocalServiceUtil.getByActiveState(Boolean.TRUE);
			renderRequest.setAttribute("districts", districtList);
			//District Sport Talent Development Centre
			List<CoachingWingFinancialDetailsReportsDTO> type1Report = getTrainingCenterFinancialReportByTrainingCenterType("taluka");
			_log.info("districts :::" +districtList);
			renderRequest.setAttribute("centerType","taluka");
			 renderRequest.setAttribute("talentDev",type1Report);
			 renderRequest.setAttribute("districts",districtList);
			return "/coaching-wing-fiancial-detail-reports.jsp";
			
		}else if(friendlyURL.equalsIgnoreCase("/sports-coaching-wing-list")) {
			_log.info("inside coaching wing list :::");
			...
			// _log.info("sportsTypes :::" +sportsTypes);
		  //List Coach Deatails
		  List<SchoolCollegeOfficerRegistration> coachList = SportsCoachingWingUtil.SchoolCollegeOfficersByDesignation(themeDisplay.getCompanyId(),RoleConstant.COACH);
			 ...
			 if(isDSO) {
				 _log.info("Entry into DSO List view :::" );
				 String mode = "add"; // default
				 List<TrainingCentre> trainingCenterList = getTrainingCenterDSOList(MhdsysSportsCoachingWingPortletKeys.STATUS_DRAFT,themeDisplay.getUserId());
				 //_log.info("trainingCenterList dso view :::: :::" +trainingCenterList);
		 			
				 	try {
		 				trainingCentre=TrainingCentreLocalServiceUtil.getByUserIdAndFormStatus(themeDisplay.getUserId(),MhdsysSportsCoachingWingPortletKeys.STATUS_DRAFT);
		 				mode = "edit";
					} catch (Exception e) {
						_log.error("Error in code ::"+e);
					}
		 			...
		 			if(Validator.isNotNull(trainingCentre)) {
						...
						getCoachDTOList(coachDTO, coachs, trainingCentre);
						...
						getFinancialDetailsDTOList(financialDetailsDtoList, trainingCentre);
				 ...
				 //convert list into dto list
				 List<TrainingCenterDto> trainingCenterDTOList=new ArrayList<>();
				 if(Validator.isNotNull(trainingCenterList) && trainingCenterList.size()>0) {
					 for(TrainingCentre tr: trainingCenterList) {
						 TrainingCenterDto t=new TrainingCenterDto();
						 t=getTrainingCenterDTO(themeDisplay, t, tr);
						 trainingCenterDTOList.add(t);
					 }
				 }
				 
				 _log.info("trainingCenterDTOList :::" +trainingCenterDTOList);
				 renderRequest.setAttribute("trainingCenterList",trainingCenterDTOList);
				 //renderRequest.setAttribute("trainingCenterList",trainingCenterList);
				 renderRequest.setAttribute("isDSO",true);
				 //new changes
				 redirectPage="/view.jsp";
				 
			 }else if(isdistdeputydirector) {
				 try {
					 List<TrainingCentre> trainingCenterList = getTrainingCenterDDList("save");
					 
					 _log.info("trainingCenterList :::" +trainingCenterList);
				 ...
				 _log.info("inside ::: HO Admin");
				 try {
					 List<TrainingCentre> trainingCenterList = getTrainingCenterAdminList("save");
					 _log.info("trainingCenterList :::" +trainingCenterList);
	 ...
	 * @return
	 */
	private List<CoachingWingFinancialDetailsReportsDTO> getTrainingCenterFinancialReportByTrainingCenterType(String centerType) {
		List<TrainingCentre> trainingCentreList=null;
		try {
			DynamicQuery trainingCenterDynamicQuery = TrainingCentreLocalServiceUtil.dynamicQuery();	
			//trainingCenterDynamicQuery.add(RestrictionsFactoryUtil.eq("centreType","talent_dev"));
			
			trainingCenterDynamicQuery.add(RestrictionsFactoryUtil.eq("centreType",centerType));
			trainingCentreList = TrainingCentreLocalServiceUtil.dynamicQuery(trainingCenterDynamicQuery);
		}catch (Exception e) {
			_log.error("Error in code ::" +e);
		}
		...
		if(Validator.isNotNull(trainingCentreList) && trainingCentreList.size()>0) {
			
			for(TrainingCentre t:trainingCentreList) {
					CoachingWingFinancialDetailsReportsDTO reportDTO=new  CoachingWingFinancialDetailsReportsDTO();
					reportDTO.setTrainingCenterId(t.getTrainingCentreId());
					
					if(Validator.isNotNull(t.getDistrict())){
						DistrictMaster districtMaster=null;
						try {
							districtMaster= DistrictMasterLocalServiceUtil.getDistrictMaster(Long.parseLong(t.getDistrict()));
						}catch(Exception e) {
					...
					try {
						reportDTO.setDistrictId(Long.parseLong(t.getDistrict()));
					} catch (Exception e) {
						_log.error("Error in code ::" +e);
					}
					List<TrainingCentreFinancialDetail> financialDetailsList=null;
					try {
						financialDetailsList=TrainingCentreFinancialDetailLocalServiceUtil.findByTrainingCentreId(t.getTrainingCentreId());
					} catch (Exception e) {
						_log.error("Error in code ::" +e);
					}
					...
					type1Report.add(reportDTO);
			}
		}
		return type1Report;
	}
	
	private List<TrainingCentre> getTrainingCenterAdminList(String formstatus) {
		List<String> statusList=new ArrayList<>(); 
		statusList.add(MhdsysSportsCoachingWingPortletKeys.APPROVED_BY_HO);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.REJECED_BY_HO);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.PENDING_WITH_HO);
		
		DynamicQuery dynamicQuery = TrainingCentreLocalServiceUtil.dynamicQuery();
		Criterion criterion1 = RestrictionsFactoryUtil.eq("formstatus", MhdsysSportsCoachingWingPortletKeys.STATUS_SUBMIT);
		Criterion criterion2 = RestrictionsFactoryUtil.eq("divdepDirDecision", Boolean.TRUE);
		Criterion criterion3 = RestrictionsFactoryUtil.in("status", statusList);
		Criterion criterion4 = RestrictionsFactoryUtil.and(criterion1, criterion2);
		dynamicQuery.add(RestrictionsFactoryUtil.and(criterion3, criterion4));
		dynamicQuery.addOrder(OrderFactoryUtil.desc("createDate"));
		return TrainingCentreLocalServiceUtil.dynamicQuery(dynamicQuery);
	}
	
	private List<TrainingCentre> getTrainingCenterDSOList(String formstatus,long userId) {
		DynamicQuery dynamicQuery = TrainingCentreLocalServiceUtil.dynamicQuery();
		List<String> statusList=new ArrayList<>(); 
		statusList.add(MhdsysSportsCoachingWingPortletKeys.APPROVED_BY_HO);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.APPROVED_BY_DD);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.REJECED_BY_DD);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.REJECED_BY_HO);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.PENDING_WITH_DD);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.PENDING_WITH_HO);
		Criterion criterion1 = RestrictionsFactoryUtil.eq("formstatus",MhdsysSportsCoachingWingPortletKeys.STATUS_DRAFT);
		Criterion criterion2 = RestrictionsFactoryUtil.in("status",statusList);
		Criterion criterion3 = RestrictionsFactoryUtil.eq("userid",userId);
	...
	private List<TrainingCentre> getTrainingCenterDDList(String formstatus) {
		List<String> statusList=new ArrayList<>(); 
		statusList.add(MhdsysSportsCoachingWingPortletKeys.REJECED_BY_HO);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.APPROVED_BY_HO);
		statusList.add(MhdsysSportsCoachingWingPortletKeys.REJECED_BY_DD);
		...
		Criterion criterion1 = RestrictionsFactoryUtil.eq("formstatus", MhdsysSportsCoachingWingPortletKeys.STATUS_SUBMIT);
		Criterion criterion2 = RestrictionsFactoryUtil.in("status",statusList);
		Criterion criterion3 = RestrictionsFactoryUtil.and(criterion1, criterion2);
	...
	private void getSportsPersonDTOList(List<SportsPersonDTO> sportsPersonDTOs, List<SportsPerson> sportsPersons,
		...
		if(Validator.isNotNull(sportsPersons) && sportsPersons.size()>0) {
			for(SportsPerson sportsPerson:sportsPersons) {
				...
				//sportsPersonDTO.setDateOfBirthString("2000-0-dd");
	...
	private TrainingCenterDto getTrainingCenterDTO(ThemeDisplay themeDisplay, TrainingCenterDto trainingCenterDTO,
			TrainingCentre trainingCentre) {
		...
		if(Validator.isNotNull(trainingCentre.getDivision())) {
			DivisionMaster divisionMaster=null;
			try {
				divisionMaster = DivisionMasterLocalServiceUtil.getDivisionMaster(Long.parseLong(trainingCentre.getDivision()));
			}catch(Exception e) {
				_log.error("Error in code :::" +e);
			}
			
			//_log.info("divisionMaster :::" +divisionMaster);
			
			trainingCenterDTO.setDivisionName(Validator.isNotNull(divisionMaster) ?divisionMaster.getDivisionName_en():StringPool.BLANK);
		}
		//Center type 
		if(trainingCentre.getCentreType().equalsIgnoreCase("talent_dev")) {
			trainingCenterDTO.setCentreTypeValue("District Sport Talent Development Centre");
		}else if(trainingCentre.getCentreType().equalsIgnoreCase("coaching")) {
			trainingCenterDTO.setCentreTypeValue("District Coaching Centre");
		} else if(trainingCentre.getCentreType().equalsIgnoreCase("khelo_india")) {
			trainingCenterDTO.setCentreTypeValue("Khelo India District Centre");
		} else if(trainingCentre.getCentreType().equalsIgnoreCase("taluka")) {
			trainingCenterDTO.setCentreTypeValue("Taluka Coaching Centre");
		} 
		DistrictMaster districtMaster=null;
		try {
		...
		// dpr document
		if (Validator.isNotNull(trainingCenterDTO.getDprDocPath()) && trainingCenterDTO.getDprDocPath().length()>0) {
		...
		if (Validator.isNotNull(trainingCenterDTO.getGeoTag()) && trainingCenterDTO.getGeoTag().length()>0) {
			...
			if(Validator.isNotNull(getGeoTagPhotosIds) && getGeoTagPhotosIds.length>0) {
				for (String getGeoTagPhotosIdStr : getGeoTagPhotosIds) {
					//_log.info("getGeoTagPhotosIdStr :::" +getGeoTagPhotosIdStr);
					try {
						...
						_log.error("Error processing Photo ID: " + e);
					}
				}
		}
